<template>
  <el-menu
    ...
    :collapse="isCollapse"
    style="height:100%;"
    active-text-color="#2089a4">
    <!-- v-if="routerMain==='main'" -->
    <!-- 首先判断是不是hidden项目，如果不是才才进入循环 -->
    <div class="list" v-for="item in routerGet" :key="item.id" v-if="(item.hidden==undefined) && (routerMapping.toString().indexOf(item.name)!==-1)">
      <!-- 循环判断假如有children则进入上面，否则进入下面 -->
      <el-menu-item v-if="item.children.length === 1" :index="item.children[0].name">
        <i :class="item.meta.icon" style="font-size:40px" />
